...
def configSetUp():
    ...
    for customer in customers_list:
        ...
        # MOCK : IF WANT TO INSERT NEW CONFIG
        ClientConfigurationDAO.insertConfigurations(customer, client_configuration_dict)
...
def tokenGenerationSetup(Configuration, customer):
    ...
    # TOKEN GENERATION FOR KAUSHALYA G
    ...
    # TOKEN GENERATION FOR SAURABH
    if Configuration['BROKER'] == 'fyers' and 'BRAHMA_V1_PRE_PRD_SD' in schema_name:
        # KG
        username = "XS25649"  # fyers_id
        password = "Today@123"  # fyers_password
        pin = "4773"  # your integer pin
        client_id = "VEMUIUOLVV-100"  # "L9NY****W-100" (Client_id here refers to APP_ID of the created app)
        secret_key = "ISOYRBOBY9"  # app_secret key which you got after creating the app
        redirect_uri = "https://myaccount.fyers.in/"  # redircet_uri you entered while creating APP.
        tokenGeneration(username, password, pin, client_id, secret_key, redirect_uri, schema_name)
    ####################################################################################################################
...
def configCleanUp():
    ...
    GlobalConfigurationDAO.deleteConfigurations(PROFILE_PROPERTIES['SCHEMA_NAME'])
    print("\n Global Configurations Clean Up Done for GLOBAL SCHEMA:: \n"+str(PROFILE_PROPERTIES['SCHEMA_NAME']))
    ...
    # POPULATE CLIENT CONFIGS AND PERFORM OPERATIONS ON EACH CUSTOMER
    for customer in customers_list:
        ClientConfigurationDAO.deleteConfigurations(customer)
        print("\n Client Configuration Clean Up Done for Customer:: \n"+str(customer))
    ####################################################################################################################
