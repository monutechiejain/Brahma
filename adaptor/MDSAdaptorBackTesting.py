...
class MDSAdaptor:
    ...
    def addBidAskToLTP(self, df_ltp):
        for i in range(0, 5):
            ...
            df_ltp.loc[bid_price] = df_ltp.loc['depth'].apply(lambda x: x['buy'][0]['price'])
            df_ltp.loc[bid_orders] = df_ltp.loc['depth'].apply(lambda x: x['buy'][0]['orders'])
            df_ltp.loc[bid_qty] = df_ltp.loc['depth'].apply(lambda x: x['buy'][0]['quantity'])

            df_ltp.loc[ask_price] = df_ltp.loc['depth'].apply(lambda x: x['sell'][0]['price'])
            df_ltp.loc[ask_orders] = df_ltp.loc['depth'].apply(lambda x: x['sell'][0]['orders'])
            df_ltp.loc[ask_qty] = df_ltp.loc['depth'].apply(lambda x: x['sell'][0]['quantity'])

    def addBidAskToMarketDepth(self, df_Market_Depth):
        for i in range(0, 5): # CHANGES DONE FOR BACKTESTING AS MARKET DEPTH ONLY RETURNS 1 MARKET DEPTH
            bid_price, bid_orders, bid_qty = 'BID_PRICE_' + str(i + 1), 'BID_ORDERS_' + str(i + 1), 'BID_QTY_' + str(i + 1)
            ask_price, ask_orders, ask_qty = 'ASK_PRICE_' + str(i + 1), 'ASK_ORDERS_' + str(i + 1), 'ASK_QTY_' + str(i + 1)
            df_Market_Depth[bid_price] = df_Market_Depth['buy'].apply(lambda x: x[0]['price'])
            df_Market_Depth[bid_orders] = df_Market_Depth['buy'].apply(lambda x: x[0]['orders'])
            df_Market_Depth[bid_qty] = df_Market_Depth['buy'].apply(lambda x: x[0]['quantity'])

            df_Market_Depth[ask_price] = df_Market_Depth['sell'].apply(lambda x: x[0]['price'])
            df_Market_Depth[ask_orders] = df_Market_Depth['sell'].apply(lambda x: x[0]['orders'])
            df_Market_Depth[ask_qty] = df_Market_Depth['sell'].apply(lambda x: x[0]['quantity'])
    ...
    def callMDSOptionChainLiteMarketDepth(self, Configuration, SP_LIST, option_type, expiry, symbol):
        ...
        while (retry < max_retries):
            ...
            try:
                mds_option_chain_lite_market_depth_url = Configuration["MDS_URL"] + '/marketDepthController'
                #logging.info("MDS OPTION CHAIN URL: {}".format(mds_option_chain_lite_market_depth_url))
                payload = {
                      "symbol": symbol,
                      "derivativeDetails": {
                        "strikePrice":SP_LIST,
                        "expiryDate": BasicCache().get('EXPIRY'),
                        "type": option_type
                      }
                    }
                option_chain_lite_dict = requests.post(mds_option_chain_lite_market_depth_url, headers={'X_AUTH': Configuration["MDS_AUTH_KEY"],
                                                                                                        'DATE_TIME': BasicCache().get('DATE_TIME'),
                                                                                                        'EXPIRY': BasicCache().get('EXPIRY'),
                                                                                                        'SYMBOL': BasicCache().get('SYMBOL')}, json=payload, timeout= 60).json()
                # TODO: Mock LTP Response
                #file_path = "/home/ec2-user/venv/python36/lib/python3.6/site-packages/oops1/src/data/mock/ltp_json_" + symbol + ".txt"
                ...
                retry = retry + 1
                if retry == max_retries:
                    raise Exception('MDS OPTION CHAIN LITE API Not Responding')
        return df_Market_Depth
    ...
    def callStrikePricesAPI(self, Configuration, symbol, expiry_date):
        ...
        while (retry < max_retries):
            ...
            try:
                mds_strike_price_url= Configuration["MDS_URL"] + '/strikeprice/?symbol='+symbol+'&expiryDate='+BasicCache().get('EXPIRY')
                logging.info("mds_strike_price_url: {}".format(mds_strike_price_url))
                strike_price_dict = requests.get(mds_strike_price_url,timeout= 60, headers={'DATE_TIME': BasicCache().get('DATE_TIME'),
                                                                                            'EXPIRY': BasicCache().get('EXPIRY'),
                                                                                            'SYMBOL': BasicCache().get('SYMBOL')}).json()
                # TODO: Mock LTP Response
                ...
                retry = retry + 1
                if retry == max_retries:
                    raise Exception('MDS STRIKE PRICE API Not Responding')
    ...
    def callHistoricalPricesAPI(self, fromDateTime, toDateTime, Configuration, symbol):
        ...
        while (retry < max_retries):
            ...
            try:
                mds_historical_price_url = Configuration["MDS_URL"] + '/historical/candles?' \
                                                                      'symbol=' + symbol + '&timeframe=' + CANDLES_TIME_FRAME + \
                                           '&startDate=' + BasicCache().get('FROM_DATE_TIME') + '&endDate=' + BasicCache().get('DATE_TIME') + '&oi=1'

                historical_price_dict = requests.get(mds_historical_price_url, timeout=60, headers={'DATE_TIME': BasicCache().get('DATE_TIME'),
                                                                                            'EXPIRY': BasicCache().get('EXPIRY'),
                                                                                            'SYMBOL': BasicCache().get('SYMBOL')}).json()
