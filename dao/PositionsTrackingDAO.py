...
from entities.PositionsTrackingTable import PositionsTrackingTable
...
def insert(schema_name, row):
    ...
    positions = PositionsTrackingTable(signal_id=row['signal_id'],
...
def getPositionsByPositionGroupId(schema_name, position_group_id):
    '''
    ...
    '''
    session = Sqlalchemy(schema_name)
    oops_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id)
...
def getActivePositionsByLatestPositionTrackingGroupIdAndSymbol(schema_name, position_group_id, symbol):
    ...
    '''
    session = Sqlalchemy(schema_name)
    max_position_tracking_group_id = session.query(func.max(PositionsTrackingTable.position_tracking_group_id)).scalar()
    positions_tracking_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                                                  PositionsTrackingTable.symbol == symbol,
                                                                  PositionsTrackingTable.position_tracking_group_id == max_position_tracking_group_id)
...
def getLatestPositionsTrackingRows(schema_name):
    '''
    Get Positions By Trans ID
    :param position_group_id:
    :return:
    ...
    session = Sqlalchemy(schema_name)
    max_position_tracking_group_id = session.query(func.max(PositionsTrackingTable.position_tracking_group_id)).scalar()
    positions_tracking_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_tracking_group_id == max_position_tracking_group_id)
    df_positions_tracking_result = pd.read_sql(positions_tracking_result.statement, session.bind)
    session.close()
...
def getActivePositionsByTransIdAndSymbol(schema_name, position_group_id, symbol):
    ...
    '''
    session = Sqlalchemy(schema_name)
    ia_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                                                  PositionsTrackingTable.symbol == symbol,
                                                                  PositionsTrackingTable.is_active == 'Y')
...
def getActivePositionsByTransIdSymbolAndOrderType(schema_name, position_group_id, symbol, orderType):
    ...
    ia_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                                                  PositionsTrackingTable.symbol == symbol,
                                                                  PositionsTrackingTable.order_type == orderType,
                                                                  PositionsTrackingTable.is_active == 'Y')
    df_positions_result = pd.read_sql(ia_positions_result.statement, session.bind)
    session.close()
    return df_positions_result
...
def getActivePositionsByTransIdSymbolAndExpiry(schema_name, position_group_id, symbol, expiryDate):
    ...
    oops_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                                                  PositionsTrackingTable.symbol == symbol,
                                                                  PositionsTrackingTable.contract_expiry_date == expiryDate,
                                                                  PositionsTrackingTable.is_active == 'Y')
    df_positions_result = pd.read_sql(oops_positions_result.statement, session.bind)
    session.close()
    return df_positions_result

def getActivePositionsByPositionGroupId(schema_name, position_group_id):
    '''
    Get Positions By Trans ID ONLY ACTIVE POSITIONS
    :param position_group_id:
    :return:
...
def getNonActivePositionsAll(schema_name):
    ...
    '''
    session = Sqlalchemy(schema_name)
    oops_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.is_active == 'N')
    df_positions_result = pd.read_sql(oops_positions_result.statement, session.bind)
    session.close()
    return df_positions_result
...
def getPositionsAll(schema_name):
    ...
    '''
    session = Sqlalchemy(schema_name)
    positions_result = session.query(PositionsTrackingTable)
...
def getNonActivePositionsByTransIdAndInstrumentType(schema_name, position_group_id, instrument_type):
    session = Sqlalchemy(schema_name)
    oops_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                                                   PositionsTrackingTable.instrument_type == instrument_type,
                                                                   PositionsTrackingTable.is_active == 'N')
    df_positions_result = pd.read_sql(oops_positions_result.statement, session.bind)
    session.close()
    return df_positions_result
...
def getActivePositionsBySymbol(schema_name, symbol):
    ...
    '''
    session = Sqlalchemy(schema_name)
    ia_positions_result = session.query(PositionsTrackingTable).filter(PositionsTrackingTable.symbol == symbol,
                                                                  PositionsTrackingTable.is_active == 'Y')
    df_positions_result = pd.read_sql(ia_positions_result.statement, session.bind)
    session.close()
    return df_positions_result
...
def updatePositionsPostSquareOffJob(schema_name, row_position):
    ...
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == row_position['position_group_id'],
                                              PositionsTrackingTable.symbol == row_position['symbol'],
                                              PositionsTrackingTable.strike_price == row_position['strike_price'],
                                              PositionsTrackingTable.instrument_type == row_position['instrument_type'],
                                              PositionsTrackingTable.contract_expiry_date == row_position['contract_expiry_date'],
                                              PositionsTrackingTable.is_active == 'Y').\
...
def updatePositionsMergeValues(schema_name, row_position):
    ...
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == row_position['position_group_id'],
                                              PositionsTrackingTable.symbol == row_position['symbol'],
                                              PositionsTrackingTable.contract_expiry_date == row_position['contract_expiry_date'],
                                              PositionsTrackingTable.strike_price_put == row_position['strike_price_put'],
                                              PositionsTrackingTable.is_active == 'Y').\
...
def updatePositionsPnlPostSquareOffJob(schema_name, row_position):
    ...
    session = Sqlalchemy(schema_name)
    current_date = datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M:%S')
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.oops_order_id == row_position['oops_order_id']).\
...
def updatePositionsPnlSquareOffException(schema_name, position_group_id, symbol, is_active, net_pnl, brokerage, squareoff_params):
    ...
    session = Sqlalchemy(schema_name)
    current_date = datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M:%S')
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                              PositionsTrackingTable.symbol == symbol,
                                              PositionsTrackingTable.is_active == 'Y'). \
...
def updatePositionsExistingOptions(schema_name, row_position):
    ...
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == row_position['position_group_id'],
                                              ...
                                              PositionsTrackingTable.strike_price == row_position['strike_price'],
                                              PositionsTrackingTable.instrument_type == row_position['instrument_type'],
                                              PositionsTrackingTable.order_type == row_position['order_type'],
                                              PositionsTrackingTable.is_active == 'Y').\
        update({'execution_price': row_position['execution_price'],
                'execution_price_actual': row_position['execution_price_actual'],
                'broker_order_id_fresh' : row_position['broker_order_id_fresh'],
...
def updatePositionsExistingOrderGenJob(schema_name, row_position, symbol):
    ...
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == row_position['position_group_id'],
                                                  PositionsTrackingTable.symbol == row_position['symbol'],
                                                  PositionsTrackingTable.symbol == symbol,
                                                  PositionsTrackingTable.ia_options_pair_id == row_position['ia_options_pair_id'],
                                                  PositionsTrackingTable.is_active == 'Y').\
        update({'execution_price_call': row_position['execution_price_call'],
                'execution_price_call_actual': row_position['execution_price_call_actual'],
                'execution_price_put' : row_position['execution_price_put'],
...
def updatePositionsQtyMF(schema_name, row_position):
    ...
    session = Sqlalchemy(schema_name)
    current_date = datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M:%S')
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.oops_positions_id == row_position['oops_positions_id']).\
...
def deletePositionsByTransIdAndSymbol(schema_name, position_group_id, symbol):
    ...
    session = Sqlalchemy(schema_name)
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id,
                                                  PositionsTrackingTable.symbol == symbol).delete()

    session.commit()
    session.close()


def deletePositionsByPositionGroupId(schema_name, position_group_id):
    '''
    Delete from Positions Table using TransId
    :param symbol:
    :param frequency:
    ...
    '''
    session = Sqlalchemy(schema_name)
    session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_group_id == position_group_id).delete()
...
def deletePositionsByPositionIds(schema_name, position_id_list):
    ...
    for position_id in position_id_list:
        session.query(PositionsTrackingTable).filter(PositionsTrackingTable.position_tracking_id == position_id).delete()

    session.commit()
    session.close()

def deletePositionsTrackingAll(schema_name):
    '''
    Delete from Positions Table using TransId
    :param symbol:
    :param frequency:
    ...
    '''
    session = Sqlalchemy(schema_name)
    session.query(PositionsTrackingTable).delete()
